...
function generate_next_word(hs, prev_word_idx)
  local prev_word
  if opt.gpuid >= 0 then
  ...
  local prediction, s_cur = unpack(dec_rnn_unit:forward({prev_word, hs}))
    ...
    while true do
      ...
      for i = 1, #beam_list do
        ...
        if last_word == form_manager:get_symbol_idx('<E>') then
          ...
          local w_new_list, p_list, s_cur = generate_next_word(h.s, last_word)
