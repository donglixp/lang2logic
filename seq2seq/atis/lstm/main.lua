...
function eval_training(param_x_)
  ...
  for i = 1, dec_max_len do
    softmax_predictions[i], model.dec_s[i] = unpack(model.dec_rnns[i]:forward({dec_batch[{{}, i}], model.dec_s[i - 1]}))
  ...
  for i = dec_max_len, 1, -1 do
    ...
    local tmp = model.dec_rnns[i]:backward({dec_batch[{{}, i}], model.dec_s[i - 1]},
                                       {crit_dx, model.ds})[2]
