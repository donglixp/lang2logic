...
function narrow_list2(tensor_list, first, last)
  local out1 = {}
  local out2 = {}
  first = first or 1
  last = last or #tensor_list
  for i = first, last do
    table.insert(out1, tensor_list[i][1])
    table.insert(out2, tensor_list[i][2])
  end
  return {out1,out2}
end

function combine_list2(tmp1, tmp2)
  local tmp = {}
  for i = 1, #tmp1 do
    table.insert(tmp, {tmp1[i], tmp2[i]})
  end
  return tmp
end
