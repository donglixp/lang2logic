...
function create_enc_lstm_unit(w_size)
  ...
  local i = {[0] = nn.LookupTable(w_size, opt.rnn_size)(x):annotate{name='enc_lookup'}}
  local next_s = {}
...
function create_dec_lstm_unit(w_size)
  ...
  local i = {[0] = nn.LookupTable(w_size, opt.rnn_size)(x):annotate{name='dec_lookup'}}
  local next_s = {}
  local splitted = {prev_s:split(2 * opt.num_layers)}
  for layer_idx = 1, opt.num_layers do
    local prev_c = splitted[2 * layer_idx - 1]
...
function setup()
  ...
  param_x, param_dx = combine_all_parameters(model.enc_rnn_unit, model.dec_rnn_unit)
...
function eval_training(param_x_)
  ...
  while (cur_index <= max_index) do
    ...
    for i = 1, dec_batch[cur_index]:size(2) - 1 do
      ...
      softmax_predictions[cur_index][i], model.dec_s[cur_index][i] = unpack(model.dec_rnns[i_dec_rnns]:forward({dec_batch[cur_index][{{}, i}], model.dec_s[cur_index][i - 1], parent_h}))
  ...
  for cur_index = max_index, 1, -1 do
    ...
    for i = dec_batch[cur_index]:size(2) - 1, 1, -1 do
      ...
      local _, tmp, parent_h_ds = unpack(model.dec_rnns[i_dec_rnns]:backward({dec_batch[cur_index][{{}, i}], model.dec_s[cur_index][i - 1], parent_h}, {crit_dx, model.ds}))
...
function main()
  ...
  -- model params
  cmd:option('-rnn_size', 250, 'size of LSTM internal state')
  ...
  cmd:option('-learning_rate',0.006,'learning rate')
  cmd:option('-init_weight',0.08,'initailization weight')
  cmd:option('-learning_rate_decay',0.975,'learning rate decay')
  ...
  for i = 1, iterations do
    ...
    if (opt.opt_method == 0) or (opt.opt_method == 1) then
