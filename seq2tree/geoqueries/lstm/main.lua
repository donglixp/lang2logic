...
function eval_training(param_x_)
  ...
  while (cur_index <= max_index) do
    ...
    for i = 1, dec_batch[cur_index]:size(2) - 1 do
      ...
      softmax_predictions[cur_index][i], model.dec_s[cur_index][i] = unpack(model.dec_rnns[i_dec_rnns]:forward({dec_batch[cur_index][{{}, i}], model.dec_s[cur_index][i - 1]}))
  ...
  for cur_index = max_index, 1, -1 do
    ...
    for i = dec_batch[cur_index]:size(2) - 1, 1, -1 do
      ...
      local tmp = model.dec_rnns[i_dec_rnns]:backward({dec_batch[cur_index][{{}, i}], model.dec_s[cur_index][i - 1]}, {crit_dx, model.ds})[2]
...
function main()
  ...
  cmd:option('-dropoutrec',0.2,'dropout for regularization, used after each c_i. 0 = no dropout')
  cmd:option('-enc_seq_length',40,'number of timesteps to unroll for')
  cmd:option('-dec_seq_length',120,'number of timesteps to unroll for')
  cmd:option('-batch_size',20,'number of sequences to train on in parallel')
  cmd:option('-max_epochs',110,'number of full passes through the training data')
  -- optimization
